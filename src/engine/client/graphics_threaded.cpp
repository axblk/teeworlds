...
void CGraphics_Threaded::FlushVertices()
{
	...
	if(m_Drawing == DRAWING_QUADS)
		RenderVertices(m_aVertices, NumVerts/4, CCommandBuffer::PRIMTYPE_QUADS);
...
void CGraphics_Threaded::Rotate4(const CPoint &rCenter, CVertex *pPoints)
{
...
IGraphics::CVertexBufferHandle CGraphics_Threaded::LoadVertexBuffer(int NumVertices, const IGraphics::CVertex *pVertices, int Usage)
{
	...
	if(pVertices)
	{
		Cmd.m_pData = (IGraphics::CVertex *)m_pCommandBuffer->AllocData(MemSize);
	...
	if(pVertices)
		mem_copy(Cmd.m_pData, pVertices, MemSize);
