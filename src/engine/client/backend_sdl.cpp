...
static const GLchar *s_pFragmentMSDFShader330 =
	...
	*/
	/*
	"    vec2 uv = v_TexCoord.xy * textureSize(s_Texture, 0);\n"
	"    vec2 Jdx = dFdx(uv);\n"
	"    vec2 Jdy = dFdy(uv);\n"
	"    vec3 sample = texture(s_Texture, v_TexCoord.xy).rgb;\n"
	"    float sigDist = median(sample.r, sample.g, sample.b) - 0.5;\n"
	...
	*/
	...
	"    float dx = dFdx(v_TexCoord.x) * texSize.x;\n"
	"    float dy = dFdy(v_TexCoord.y) * texSize.y;\n"
	"    float toPixels = 8.0 * inversesqrt(dx * dx + dy * dy);\n"
	"    float sigDist = median(sample.r, sample.g, sample.b) - 0.5;\n"
	"    float opacity = clamp(sigDist * toPixels + 0.5, 0.0, 1.0);\n"
	"    float mu = clamp(sigDist * toPixels + 0.45, 0.0, 1.0);\n"
	"    vec3 rgb = mix(vec3(0,0,0), v_Color.rgb, mu);\n"
	"    f_Color = vec4(rgb, max(opacity,mu));\n"
